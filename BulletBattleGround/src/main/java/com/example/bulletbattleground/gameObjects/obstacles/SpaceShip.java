...
public class SpaceShip extends Obstacle {
    ...
    public static final double SPACESHIP_TIP_HEIGHT = 10;
    public static final double SPACESHIP_TIP_LENGTH = 25;
    public static final double SPACESHIP_MASS = 190000;
    private static final double SPACESHIP_BOUNDS_Y = 160;

    public SpaceShip(int speed, int coordinateX, int coordinateY) {

        double x1 = coordinateX - DEFAULT_WIDTH / 2, y1 = coordinateY - DEFAULT_HEIGHT / 2 - SPACESHIP_TIP_LENGTH;
        double x2 = coordinateX + DEFAULT_WIDTH / 2, y2 = coordinateY - DEFAULT_HEIGHT / 2;
        double x3 = coordinateX + DEFAULT_WIDTH / 2, y3 = coordinateY + DEFAULT_HEIGHT / 2 - SPACESHIP_TIP_HEIGHT;
        double x4 = coordinateX + DEFAULT_WIDTH / 2 + SPACESHIP_TIP_HEIGHT, y4 = coordinateY + DEFAULT_HEIGHT / 2;
    ...
    public void move(double dt) {
        ...
        if (getCoordinate().getY() <= -SPACESHIP_BOUNDS_Y || getCoordinate().getY() > 1200/*TODO max width of screen parameter*/) {
    ...
    public void allign(){
        if (getVelocityY() > 0.0) {
            double x1 = getCoordinate().getX() + DEFAULT_WIDTH / 2, y1 = getCoordinate().getY() + DEFAULT_HEIGHT / 2 + SPACESHIP_TIP_LENGTH;
            double x2 = getCoordinate().getX() - DEFAULT_WIDTH / 2, y2 = getCoordinate().getY() + DEFAULT_HEIGHT / 2;
            double x3 = getCoordinate().getX() - DEFAULT_WIDTH / 2, y3 = getCoordinate().getY() - DEFAULT_HEIGHT / 2 + SPACESHIP_TIP_HEIGHT;
            double x4 = getCoordinate().getX() - DEFAULT_WIDTH / 2 - SPACESHIP_TIP_HEIGHT, y4 = getCoordinate().getY() - DEFAULT_HEIGHT / 2;
